...
<main class="flex">
  ...
  >
          ...
          {#each tocItems as item (item.href)}
            <li style={getIndentStyle(item.level)}>
              <div class="flex items-center">
                ...
                {/if}
                <!-- eslint-disable svelte/no-navigation-without-resolve -->
                <a class="hover:text-rose-500" href={item.href}>{item.text}</a>
                <!-- eslint-enable svelte/no-navigation-without-resolve -->
              </div>
            </li>
          {/each}
    ...
    <div
      bind:this={article}
      class="prose-invert prose py-4 prose-rose max-w-none prose-headings:scroll-m-20"
    >
      {@render children?.()}
    </div>
  ...
  <!-- Desktop Sidebar TOC -->
  <aside
    class="w-64 flex-1 sticky top-14 h-[calc(100vh-3.5rem)] max-h-[calc(100vh-3.5rem)] grow min-w-fit hidden lg:flex"
  >
                ...
                <div class="flex items-center">
                  ...
                  <a
                    ...
                    class={{
                      ...
                      "text-rose-500": currentHeading === item.href.slice(1),
                    }}>{item.text}</a
                  >
                  <!-- eslint-enable svelte/no-navigation-without-resolve -->
                </div>
